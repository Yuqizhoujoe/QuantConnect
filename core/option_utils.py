...
class OptionContractSelector:
    ...
    @staticmethod
    def filter_by_expiry_window(contracts: List[Any], expiry_window: Tuple[date, date]) -> List[Any]:
        """Filter contracts by expiry window."""
        return [c for c in contracts 
                if expiry_window[0] <= c.Expiry.date() <= expiry_window[1]]
    
    @staticmethod
    def filter_by_delta_range(contracts: List[Any], delta_range: Tuple[float, float], 
                            get_delta_func) -> List[Any]:
        """Filter contracts by delta range."""
        delta_min, delta_max = delta_range
        return [c for c in contracts 
                if delta_min <= abs(get_delta_func(c)) <= delta_max]
    ...
    def filter_by_frequency(contracts: List[Any], frequency: str) -> List[Any]:
        ...
        return [c for c in contracts if OptionAnalysis.is_valid_option_expiry(c.Expiry, frequency)]
    
    @staticmethod
    def select_best_contract(valid_contracts: List[Any], underlying_price: float,
                           market_analysis: Dict[str, Any], target_delta_range: Tuple[float, float],
                           get_delta_func) -> Optional[Any]:
        ...
        for contract in valid_contracts:
            ...
            regime = market_analysis.get('market_regime', 'neutral')
    ...
    @staticmethod
    def get_available_deltas(contracts: List[Any], expiry_window: Tuple[date, date],
                           get_delta_func) -> Tuple[float, float]:
        """Get available delta range for contracts in expiry window."""
        filtered_contracts = OptionContractSelector.filter_by_expiry_window(contracts, expiry_window)
...
class OptionTradeLogger:
    ...
    @staticmethod
    def log_trade_entry(algorithm: Any, contract: Any, quantity: int, 
                       market_analysis: Dict[str, Any], get_delta_func) -> None:
        """Log trade entry information."""
        algorithm.Log(f"Market conditions: {market_analysis.get('market_regime', 'unknown')}, "
                     f"Volatility: {market_analysis.get('volatility', {}).get('regime', 'unknown')}, "
                     f"RSI: {market_analysis.get('rsi', 0):.1f}")
        
        algorithm.Log(f"Sold short put: {contract.Symbol.Value}, "
    ...
    @staticmethod
    def log_no_valid_contracts(algorithm: Any, market_analysis: Dict[str, Any],
                             target_delta: Tuple[float, float], available_deltas: Tuple[float, float]) -> None:
        """Log when no valid contracts are found."""
        algorithm.Log(f"No valid puts found. Market: {market_analysis.get('market_regime', 'unknown')}, "
                     f"Target delta: {target_delta[0]:.2f}-{target_delta[1]:.2f}, "
                     f"Available: {available_deltas[0]:.2f}-{available_deltas[1]:.2f}")
    
    @staticmethod
    def create_trade_record(contract: Any, quantity: int, price: float, 
                          get_delta_func, current_date: date) -> Dict[str, Any]:
        ...
        return {
            ...
            'underlying_price': getattr(contract, 'UnderlyingLastPrice', contract.Strike)
        } 